...
//                || (obj instanceof File)
    ...
    public static String classToCLIString(Class<?> aClass, Class<?> requiredType) {
		...
		//TEST
		System.out.println("Class to CLI class name is:" + className);
        String packageName = requiredType.getPackage().getName();
		System.out.println("Class to CLI required type is:" + packageName);
		System.out.println("packageNameLength is: " + packageName.length()); 
        if (className.startsWith(packageName)) {
            // cut off package name
			//Original substring
            //className = className.substring(packageName.length() + 1, className.length());
			className = className.substring(packageName.length(), className.length());
